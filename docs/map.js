...
const BRIEF=[
  ...
  ["2 · Downstream exposure",
   "Exposure is the built-up land and cropland inside the modelled 100-year river flood extent (JRC/CEMS global river flood hazard maps). The maps model no flood protection, so the extent is the floodplain the dikes defend, not the residual risk behind them. Each km² counts equally. This is the demand that upstream retention serves. The extent follows the Fraser and its tributaries, so much of the region's higher ground carries no exposure."],
  ["3 · Linking the two",
...
const LIMITS={
  ...
  limitations:[
    ...
    ["Cross-border watersheds do not line up exactly",
     "Canada and the United States survey their watersheds separately, and the two do not divide the border the same way. Where the two overlap the Canadian delineation is kept and the American one trimmed, so no ground is counted twice, but a watershed the border divides can still read as cut. Fishtrap Creek is the clearest case, where one American watershed covers several Canadian ones. The technical report sets out the rest."],
    ...
    ["Population does not drive the ranking",
     "Exposure is measured as land area at every stage, so a densely populated floodplain and a sparsely populated one of equal area count the same. Across the exposed neighbourhoods, built-up area and population are effectively uncorrelated, so land area does not stand in for people. Population is tallied after the ranking, to describe who lives downstream of it. The population slider under Supply &amp; demand is a trial control: raised, it asks which ecosystems serve people rather than land."],
    ["Community boundaries are administrative",
     "A community is credited with the watersheds upstream of every demand basin its boundary enters. Those boundaries follow jurisdiction rather than drainage, so a member drawn in several pieces can hold a small detached parcel in a main-stem watershed and be credited with the entire network above it. Metro Vancouver A and Fraser Valley B read this way: their exposure figures are unaffected, being measured inside each boundary, but their upstream counts describe reach rather than footprint."],
